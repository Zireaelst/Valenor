...
async function main() {
  ...
  if (args.length < 2) {
    console.error("Usage: npx hardhat run scripts/faucetTestUSDC.ts --network <network> -- <recipient> <amount>");
    console.error("Example: npx hardhat run scripts/faucetTestUSDC.ts --network sepolia -- 0x1234... 1000");
    process.exit(1);
  }
  ...
  try {
    ...
    const testUSDCAddress = process.env.USDC_TOKEN_ADDRESS;
    if (!testUSDCAddress) {
